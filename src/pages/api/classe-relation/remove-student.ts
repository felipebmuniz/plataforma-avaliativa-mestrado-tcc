...
const fetchClassesRemoveStudent: NextApiHandler = async function (
  request,
  response,
) {
  ...
  const url =
    baseURL +
    `/Classes/${body?.data?.classId}/remove-student/${body?.data?.studentId}`;
...
export default fetchClassesRemoveStudent;
