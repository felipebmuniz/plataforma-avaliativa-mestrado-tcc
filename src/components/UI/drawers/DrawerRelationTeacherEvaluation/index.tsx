...
export const DrawerRelationTeacherEvaluation = ({
  classId,
}: {
  classId: string;
}) => {
  const { relationEvaluationTeacher } = useClasses();
  ...
  function onSubmit(values: { evaluations: string[] }) {
    values.evaluations.map((evaluation) => {
      ...
      relationEvaluationTeacher(aux, clearForm);
    });
  }
  ...
  return (
    <>
      <MenuItem
        ...
        onClick={onOpen}
      >
        Vincular Avaliação para o Docente
      </MenuItem>
      ...
      >
        ...
        <DrawerContent>
          ...
          <DrawerHeader borderBottomWidth="1px">
            Criar Relação do Docente da Turma com a Avaliação
          </DrawerHeader>
